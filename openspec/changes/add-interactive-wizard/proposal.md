...
## Why
The `canopy workspace new` command requires users to remember flag names and syntax (--repos, --branch, --template, etc.). For new users or infrequent workspace creators, an interactive wizard provides a friendlier experience with prompts, validation, and contextual help. This leverages the existing Bubble Tea TUI infrastructure for consistency.

## What Changes
- Add `canopy workspace create` command (alias for interactive mode, distinct from `new`)
...
- Add `--wizard` flag to `canopy workspace new` to enter wizard mode
...
- Affected code:
  ...
  - `cmd/canopy/workspace.go` - Add create command and --wizard flag
