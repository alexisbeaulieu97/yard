...
## What Changes
- Add `~/.canopy/repos.yaml` registry file for storing repository aliases with metadata
- Implement `RepoRegistry` struct in `internal/config/` to manage registry operations
- Add new commands: `canopy repo register <alias> <url>`, `canopy repo unregister <alias>`, `canopy repo list-registry`
...
- Auto-sync registry on `canopy repo add` to create aliases from cloned repos
...
- Affected code:
  ...
  - `cmd/canopy/repo.go` - Add register/unregister commands
  - `cmd/canopy/workspace.go:29` - Accept registry aliases in --repos flag
