...
- [x] 1.3 Add registry file path resolution (~/.canopy/repos.yaml)
...
- [x] 4.4 Update error messages to suggest "canopy repo register" for unknown aliases

## 5. Add Registry Commands
- [x] 5.1 Implement `canopy repo register <alias> <url>` command
- [x] 5.2 Add --branch, --description, --tags flags to register command
- [x] 5.3 Implement `canopy repo unregister <alias>` command
- [x] 5.4 Implement `canopy repo list-registry` command with formatting
- [x] 5.5 Add `canopy repo show <alias>` to display registry entry details

## 6. Auto-Registration Feature
- [x] 6.1 Update `canopy repo add <url>` to auto-register with derived alias
...
- [x] 7.2 Update `canopy repo --help` with registry commands
- [x] 7.3 Add colored output for registry list command
- [x] 7.4 Include registry file location in `canopy check` output
