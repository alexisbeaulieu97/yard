...
## What Changes
- Add template definitions to `~/.yard/config.yaml` under `templates:` section
- Implement template resolution in workspace creation command
- Add `--template <name>` flag to `yard workspace new` command
- Add `yard template list` command to show available templates
...
- Affected code:
  ...
  - `cmd/yard/workspace.go:23-70` - Add --template flag to new command
  - `cmd/yard/main.go` - Add `yard template` command group (optional)
